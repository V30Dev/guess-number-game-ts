const input = document.querySelector('#attempt-input') as HTMLInputElement;
const guess = document.querySelector('#guess-button') as HTMLButtonElement;
const reset = document.querySelector('#reset-button') as HTMLButtonElement;
const feedBackContainer = document.querySelector('.card__feedback') as HTMLDivElement;
const feedbackText = document.querySelector('#feedback-text') as HTMLParagraphElement;
const cardForm = document.querySelector('.card__form') as HTMLParagraphElement;
const progressBar = document.querySelector('#progress-bar') as HTMLDivElement;
const attemptText = document.querySelectorAll('.card__attempt');


const generateRandomNumber = (): number => Math.floor(Math.random() * (101 - 1) + 1);
...
const setFeedback = (text: string, type?: "low" | "high" | "win") => {
